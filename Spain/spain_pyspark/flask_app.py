...
def insert_data_in_news_elastic():
    ...
    df = df.drop('image').drop('sourceName')\
        .withColumnRenamed('description', 'body').withColumnRenamed('url', 'link').withColumnRenamed('time', 'date1')
    df = df.withColumn('date', regexp_replace(substring(df['date1'], 0, 10), r'\.', r'-')).drop('date1')
