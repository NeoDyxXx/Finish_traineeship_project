...
def visa_to_es():
    ...
    for i in range(len(center2)):
        ...
        body = {}
        body['analyzer'] = 'rebuilt_keyword'
        body['text'] = center2[i]['Адрес']
        tokens = client.indices.analyze(index="visaac", body=body)
        index = tokens['tokens'][0]['token']
        ...
        doc = {}
        doc['country'] = center2[i]['Страна']
        doc['address'] = address
        doc['index'] = index
        doc['email'] = center2[i]['почта']
        doc['issue_worktime'] = center2[i]['Время выдачи паспортов']
        doc['apply_worktime'] = center2[i]['Время приема']
        doc['telephone1'] = center2[i]['Тел']
        client.index(index="visaac", id=i + 1 + len(center), document=doc)
        ...
        q = {
            "bool": {
                "must": [
                    ...
                    {"match" : {"body":  item['description']}},
                    {"match" : {"date":  item['time'][:10].replace('.', '-')}},
                    {"match" : {"link":  item['url']}}
                ]
            }
        }
